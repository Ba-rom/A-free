...
class TfPoseVisualizer:
    ...
    def draw_pose_rgb(npimg, humans, imgcopy=False):
        ...
        for human in humans:
            xs, ys, centers = [], [], {}
            # Draw all relevant nodes onto image
            for i in range(CocoPart.Background.value):
                if i not in human.body_parts.keys():

                    # for missing data, add 0
                ...
                cv.circle(npimg, center, 3, CocoColors[i], thickness=TfPoseVisualizer.Thickness_ratio * 2,
                          lineType=8, shift=0)
            # Connect the joints of the same person according to each part
            ...
            # Generate ROI area based on each person's joint point information
            tl_x = min(xs)
            tl_y = min(ys)
            width = max(xs) - min(xs)
            ...
            # Record all related nodes in each frame
            joints.append(centers)

            # Record point 1 of coco as Xcenter
            if 1 in centers:
